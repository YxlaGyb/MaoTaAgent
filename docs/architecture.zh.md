...
| `packages/hooks` | 四个 hook 点：持有那四个事件的方言包，以及发现 hook 插件、合并它们答案的引擎。 | [四个 hook 点](user/hooks.zh.md) |
...
| `packages/*` | 内核插件：api、pwsh-local、permission、tool-pwsh、tool-fs、tool-fs-search、tool-todo、tool-subagent、tools、skill、skill-filesystem、session、hooks 与 agent。 | `packages/<组>/<包>/src/index.ts`，例如 `packages/agent/agent-core/src/index.ts` |
