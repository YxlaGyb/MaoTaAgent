...
`agent` 插件是用户真正对话的那个内核插件：它把一段已存会话加一条输入变成模型与工具的循环，并把结果流式送回。它在这里分成两半：读配置、拼提示词、管会话的插件侧，以及调用模型、执行工具的循环引擎。先在这一页挑对你要动的那一半，再进对应目录。
...
profile 配置拉起的是 `@maota/agent-core`，由 `agent-core/src/index.ts` 构建。`agent-loop` 由那个入口 import，从不被单独拉起。
