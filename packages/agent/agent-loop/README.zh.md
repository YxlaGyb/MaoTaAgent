...
两条接缝都会收到一个 `StepContext`。

| 成员 | 含义 |
|---|---|
| `state` | `{ messages, step, maxSteps, lastReason }`。`state.messages` 是一次运行中消息的唯一真相。 |
...
| `steps` | 已发起的模型调用数。 |
| `text` | 最后的助手文本，没有时为空。 |
| `reason` | `completed`、`aborted` 或 `max_steps`。 |
...
只要助手消息里带工具调用，循环就继续；它从不读取 `stop_reason`，因为流式回答可能在那个字段落定之前就已经带上了工具调用。每一步先发 `step`，调用 `chat`，追加助手消息，若没有带回工具调用就以 `completed` 结束。否则把调用交给 `execute.ts`，然后再检查一次信号。信号在一轮开始前和工具轮结束后各查一次，两种情况停下来都报 `aborted`。
