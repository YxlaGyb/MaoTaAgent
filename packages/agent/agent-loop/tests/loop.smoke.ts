...
/// A decision is not advice: what the pre-tool seam says the arguments are is
/// what the tool is handed, what is classified and what the result is written
/// against, and what the post-tool seam says the answer is is what the model
/// reads.
{
  const seen: unknown[] = [];
...
/// A seam that rewrites nothing changes nothing, and one that throws is not the
/// layer that decides.
{
  const messages: Message[] = [{ role: "user", content: "go" }];
  await runLoop(
