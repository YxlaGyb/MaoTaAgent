...
读写文件是三个包：一个决定“一条路径算什么意思”的库，和两个把那个库变成模型可见工具的插件。库持有工作区根、包含判定与原子写，被 import 而不被拉起。插件被拉起，答 `tool.read`、`tool.write`、`tool.edit` 与 `tool.glob`，彼此之间一无所知。先在这一页挑对包，再打开它的目录看契约。
...
| `@maota/tool-fs-search` | [`tool-fs-search`](tool-fs-search/) | `tool.glob` | 按路径模式找文件的插件。 |
