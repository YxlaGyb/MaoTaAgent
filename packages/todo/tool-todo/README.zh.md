...
- **一个工具，一份整清单**：没有逐项的 create、update、get，也没有项与项之间的依赖图。这两样属于更广的生态从这个版本长出来的任务系统，不属于这一版。
- **不数 `in_progress`**：可以同时有多项在进行中，这正是并行工作需要的样子。描述里建议只留一项，没有任何东西强制。
- **提醒只是建议**：跳过验证的模型不会被拦住，事后也不会有人再读一遍计划。
- **没有前端**：计划在会话文档里，也在工具结果里，没有任何事件发布出去供面板跟读。
- **计划随会话一起消失**：没有计划文件，也没有跨会话的清单，所以删掉会话就删掉了它的计划。
