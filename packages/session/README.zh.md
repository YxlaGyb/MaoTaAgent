...
存下来的对话：每个会话、每个工作目录一个 JSON 文档，装着这一轮的消息、会话在列表里显示用的标题，以及模型持有那份计划。六个方法回答调用方：`list`、`load`、`save`、`delete`，以及计划那一对 `todos` 与 `save_todos`。文档是版本 2，整份写进临时文件再改名就位，写入过程按会话上锁。保存消息永远不会丢掉计划，版本 1 的文档在读入时被规范化，再写回时就是版本 2。
...
| 方法 | 参数 | 返回 |
|---|---|---|
| `list` | 无 | `{ dir, sessions }`，按 `updated_at` 倒序，最新的在前。 |
| `load` | `{ id, cwd }` | 文档，并把 `dangling` 算出来。从未写过的会话读出来是一份空会话。 |
| `save` | `{ id, cwd, title?, messages }` | 写下去的那份文档。 |
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 能力本体：配置、六个方法与 `selfCheck`。 |
| [`src/store.ts`](src/store.ts) | 文档层：路径、读取、原子写入，以及计划的读取与追加。 |
...
`load` 接受 `schema_version` 为 1 的文档，补上一个空的 `events`；完全没有版本字段的文档同样处理。其余情况按读不动的文件处理，损坏的文档也是这个下场。读过版本 1 之后的第一次 `save` 会把文档写回版本 2，所以迁移只花一次写入，不需要单独的命令。形状不被本版本信任的事件会被丢掉，文档的其余部分保留。
