...
存下来的对话：每个会话、每个工作目录一个 JSON 文档，装着这一轮的消息、会话在列表里显示用的标题，以及模型持有那份计划。八个方法回答调用方：`list`、`load`、`save`、`delete`、`children`、计划那一对 `todos` 与 `save_todos`，以及 `events`。文档是版本 4，整份写进临时文件再改名就位，写入时的锁，指向同一目录的第二个 host 同样认。保存消息永远不会丢掉计划，版本 1、2 或 3 的文档在读入时被规范化，再写回时就是版本 4。文档可以带一条父链，子代理的会话就是这样存的：它留在父的目录里，`children` 与 `list` 都带着这条链。
...
| `events` | `{ id, cwd }` | `{ events, todos }`：文档留下的那些计划事件，以及从它们折出来的投影。 |

### 文档

| 字段 | 含义 |
|---|---|
| `schema_version` | 本版本写出的每份文档都是 `4`。 |
| `id` | 会话 id。 |
| `cwd` | 这个会话所属的工作目录，去掉结尾的分隔符。 |
| `title` | `list` 显示的内容。`save` 不带它时，保留已经存着的标题。 |
| `created_at` | 文档第一次被写下的时间。 |
| `updated_at` | 最后一次被写下的时间，两条写入路径都算。 |
| `messages` | 消息记录：就是 `agent-core` 组装并交回来的那个数组。消息可以带一个 `source`，例如一条技能目录注记；`messageSource` 在文件边界上校验它，并丢掉本版本读不动的值。 |
...
子文档住在父自己的目录里，因为子代理拿到的是父的工作目录、也从不写到它外面；它的 `parent` 记着是谁派出了它、在哪个目录、挂在哪次调用下，以及那次调用带的标签。这条链接换来三个答案：`children` 返回某段会话派出的那些，最早的在最前；`list` 列出每一段会话，子代理的也在，每一项上的链接说着是谁派出了它；这条链接随索引一起走，所以重开一段旧会话的页面不加载任何东西就能把条目挂回那次调用下面。
...
| `max_events` | `1000` | 一份文档最多留多少条计划事件，再往前的就被折成一条快照。 |
...
两条写入路径都走 `serially`，键是工作目录加会话 id，所以同一个会话的计划写入与消息写入永远不会交错：各自读出文档、改自己那一部分，再把整份文件写回去。写入本身是先在会话目录里写一个临时文件再改名，权限位 `0o600`，于是读者看到的要么是写入之前的文档，要么是之后的，绝不会看到半份。也正是这把锁，让 `save` 不会丢掉在它读出与写出之间被追加的那份计划。它还落成一个文件：`<文档>.lock`，用 `wx` 建出来，里面写着写入者的 pid，于是指向同一个 `$MAOTA_HOME` 的第二个 host 会排队等，而不是 rename 掉一份它没读过的写入。主人已经不在了、或者超过三十秒的锁会被接手，并留一条告警；五秒等待结束时还被攥着的锁会被拒绝，绝不交错。
...
`load` 接受 `schema_version` 为 1、2 或 3 的文档，补上一个空的 `events` 与「没有父链」；完全没有版本字段的文档同样处理。其余情况按读不动的文件处理，损坏的文档也是这个下场。这样读过之后的第一次 `save` 会把文档写回版本 4，所以迁移只花一次写入，不需要单独的命令。形状不被本版本信任的事件或父链会被丢掉，文档的其余部分保留。

### 子会话顺着链接找出来

`childrenOf` 走一遍父自己的目录，留下 `parent.id` 正是被问那段会话的文档，先按创建时间、再按 id 排序，所以两个子会话落在同一毫秒里时答案也是稳定的。把它们归到一起的只有那条链接，所以链接丢了的文档就变回一段普通会话，而链接的好坏本就只取决于它被写进的那个文件。
...
六条事实界定了这个存储。锁是一个文件，所以指向同一个 `$MAOTA_HOME` 的两个 host 是排队而不是抢；主人死掉的锁过了陈旧窗口会被接手，等待窗口结束还被攥着的会被拒绝。事件不再无限增长：越过 `max_events` 之后旧事件折成一条 `todos.snapshot`，这也是写久了之后 `revision` 会读到上限的原因。`list` 读的是各写入方维护着的索引，所以夹在索引与调用之间落盘的那次写入要到下次重建才露面，而不是立刻。本版本读不动的文档会被改名成 `…bad-<时间戳>` 并上报，而不是读成一段新会话、再被下一次 `save` 覆盖掉。投影是最后一份快照，它背后的那些事件经 `events` 交给想要历史的调用方。两条写入路径仍然各拥有自己的字段，所以消息记录与计划谁也丢不掉谁。
